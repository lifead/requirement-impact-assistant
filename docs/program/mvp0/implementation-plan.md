...
- Task 18;
- Task 19 должна быть реализуема и проверяема без DeepSeek API key, user secrets и network access.

Входит:

- серверный экспорт Markdown из сохраненного анализа;
- возможность скачать/сохранить `.md` файл из UI анализа;
- название анализа;
- дата экспорта и текущий статус анализа;
- исходное требование, предлагаемое изменение, ситуация и источник изменения;
- список контекстных фрагментов или их метаданные/содержимое в человекочитаемом виде;
- структурированная карта влияния, включая риски, противоречия, вопросы и недостающую информацию;
- экспертная оценка, включая отметки, пропущенные элементы, корректировки, оценки достаточности контекста/полезности результата и общий комментарий;
- экспертное заключение, комментарий, основание, дата фиксации и статус;
- пояснение, что LLM сформировала предварительный аналитический материал и не принимает управленческое решение.

Не входит:

- JSON export;
- PDF export;
- пользовательские шаблоны отчета;
- аналитика главы 4;
- dashboard;
...
- Task 19 не вызывает `IAiAnalysisEngine`, LLM provider или внешние сервисы;
- Markdown export доступен только для анализа с сохраненным `ExpertConclusion`;
- если в реализации фиксируется факт экспорта или статус `Exported`, это должно быть минимальным маркером для будущей Task 21, без полноценной snapshot protection;
- Task 19 не реализует защиту evaluated/exported snapshots и не запрещает повторный анализ; защита воспроизводимости остается scope Task 21.

Проверки:

- snapshot-style text test на ключевые разделы Markdown-отчета;
...
- Markdown-отчет можно скачать/сохранить из UI анализа только после сохраненного экспертного заключения и использовать в проектных или диссертационных материалах;
- Task 19 можно передать в реализацию без расширения scope до JSON/PDF export, пользовательских шаблонов, dashboard, аналитики главы 4, snapshot protection, rerun AI analysis, DeepSeek, сети и секретов.
